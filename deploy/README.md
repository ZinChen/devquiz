...
Прод обновляется слиянием `dev` → `main`. PR удобнее готовить скриптом
`bin/release-pr` — он проверяет то, что легко забыть: что вы в нужной ветке,
что всё закоммичено и запушено, что CI на стенде зелёный.

```bash
bin/release-pr "v0.6"       # релиз по milestone: тело — Closes #N
bin/release-pr              # без milestone: тело из списка коммитов
bin/release-pr --dry-run    # показать, что получится, и выйти
```

С milestone скрипт собирает `Closes #N` по открытым issue — без этого после
merge они останутся открытыми, а release notes выйдут пустыми. Тег и релиз
создаются автоматически при **закрытии milestone**, не при merge.
