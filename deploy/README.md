...
`deploy` (имя можно передать аргументом), отключает вход по паролю и под root,
поднимает ufw на 22/80/443, fail2ban, автообновления, 4 ГБ swap, ставит Docker,
клонирует репозиторий, ставит `dc`, заводит `.env` из шаблона и прописывает
cron для бэкапов и очистки образов.
...
Образ появляется в ghcr после первого прогона CI на `main`. Если пуш уже был
и секреты на месте — деплой пройдёт сам, этот шаг не нужен.
...
```bash
./dc ps
./dc logs -f caddy               # выпуск сертификата
